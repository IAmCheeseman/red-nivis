...
func _draw() -> void:
	var offset = vel.normalized() * 3
	var eyePositions = [
		Vector2(-3, 0) + offset,
		Vector2( 3, 0) + offset
	]
	draw_circle(Vector2.ZERO, 7, Color.white)
	for i in eyePositions:
		draw_circle(to_local((global_position + i).round()), 1, Color.black);


func explode(area: Area2D) -> void:
