...
func shoot():
	...
	for i in gun.stats.multishot:
		...
		var spread = deg2rad(gun.stats.spread*i-(gun.stats.spread*(gun.stats.multishot-1)/2))
		...
		gunSprite.rotation_degrees = gun.stats.kickUp*2.2 if gunSprite.scale.y == -1 else -gun.stats.kickUp*2.2
